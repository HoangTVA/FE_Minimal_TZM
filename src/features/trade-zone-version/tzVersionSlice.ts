...
import { convertBinaryFilterToList, convertTzVersionToEvents, selectEvent } from "utils/common";
...
export const selectedEventSelector = createSelector(selectTzVersionEvents, selectedEventId, (list, id) => (selectEvent(list, id)));

export const selectTzVersionOptions = createSelector(selectTzVersionList, (list) => list.map((el) => ({
    name: el.name,
    id: el.id
})));
//reducers
